...
sem_t * semaforo_hilos, *semaforo_cuentas,*semaforo_trans,*semaforo_log,*semaforo_fifo;
...
int main(int argc, char *argv[])
{
    ...
    char  mensaje[255];
      ...
      //Caragamos los parametros de configuracion como el nombre el maximo de transacciones y demas
    //Que se han pasado por parametro 
    ...
    User.ficheroCuentas = argv[6];
    User.ficheroLog = argv[7];
    ...
    if ( senial == SIGINT || senial == SIGHUP || senial == SIGTERM )
    {
        ...
        if ( User.operacionactiva != 0) 
        {
            ...
            if (User.operacionactiva != 0)
            {
                ...
                if ( semaforos_usados[1] == 1) sem_post(semaforo_cuentas);
...
void *Deposito(void *args)
{
    ...
    //Atualizamos los datos por si se han cambaido desde otra sesion
    actualizarDatos();
...
void *ConsultarSaldo(void *args)
{
    ...
    //Actualizamos los datos por si se han cambiado desde otra sesion
    actualizarDatos();
    ...
    sleep(3);
    //Bloqueamos el acceso a la variable global para evitar probelmas
...
void  modificarFicheroCuentas(DATA d)
{
    ...
    //Creamos las variables
    FILE *fichero,*temp;
    bool tranferencia,saltar;
    char linea[255],error[255];
    char *user,*id,*trans,*saldo;
    int aux_id,aux_saldo,aux_trans;


    saltar = true;
    //Bloqueamos el hilo de acceso al fichero de cuentas para la exclusion mutua
   
    sem_wait(semaforo_cuentas);
    semaforos_usados[1]=1;
   
    fichero = fopen(User.ficheroCuentas,"r");
    temp = fopen("tmp","a");

    //Vemos si alguno ha fallado

    if ( temp == NULL)
    {
        snprintf(error,sizeof(error),"Error %d al crear al fichero temporal de cuentas por parte del usuario con PID %d",errno,pid_programa);
        fprintf(stderr,"%s\n",error);
        escribirLog(error);
        sem_post(semaforo_cuentas);
        semaforos_usados[1]=0;
        //Si no puedo abrir el ficher temporal no puedo hacr las operaciones , por tanto me muero
        kill(pid_programa,SIGTERM);
    }

    if ( fichero == NULL)
    {
        snprintf(error,sizeof(error),"Error %d al abrir el fichero %s por parte del usuario con PID %d",errno,User.ficheroCuentas,pid_programa);
        fprintf(stderr,"%s\n",error);
        escribirLog(error);
        sem_post(semaforo_cuentas);
        semaforos_usados[1]=0;
        //Si no puedo abri el ficher de cuentas no peudo catucliar trnasasciones , asi que me muero
        kill(pid_programa,SIGTERM);
    }
    ...
    {
        //Me salto la cabecera
        if (saltar) saltar = false;

        //Quito el \n para evitar fallos
        linea[strcspn(linea,"\n")] = 0;
        //Saco los datos de cada linea
        user = strtok(linea,";");
        id = strtok(NULL,";");
        saldo = strtok(NULL,";");
        trans = strtok(NULL,";");
        ...
        if (d.idTranferencia > 0)
        {
            ...
            if (d.idCliente == aux_id )
            {
                //Actualizo los datos
                aux_saldo = atoi(saldo);
                aux_saldo -= d.cantidad;
                aux_trans = atoi(trans);
                aux_trans++;
                //Lo pongo el ficher temporal actualizado 
                fprintf(temp,"%s;%s;%d;%d\n",user,id,aux_saldo,aux_trans);
                continue;
            }
            //Este es el que recibe la tranferecnia
            if (d.idTranferencia == aux_id )
            {
                //Actualizo los datos
                aux_saldo = atoi(saldo);
                aux_saldo += d.cantidad;
        ...
        else
        {
            ...
            if (d.idCliente == aux_id )
            {
                ...
                if (d.idTranferencia == -1 ) aux_saldo -= d.cantidad;
                //Si lo mete se le suma
                else if ( d.idTranferencia == -2) aux_saldo += d.cantidad;
                aux_trans = atoi(trans);
                aux_trans++;
                //Lo pongo en el fichero temporal
                fprintf(temp,"%s;%s;%d;%d\n",user,id,aux_saldo,aux_trans);
                continue;
            }
            fprintf(temp,"%s;%s;%s;%s\n",user,id,saldo,trans);
    ...
    fclose(temp);
    //elimino el fichero origial
    remove(User.ficheroCuentas);
    //Renombro el temporal al original 
    rename("tmp",User.ficheroCuentas);
...
// Retorno: void*
// Parámetros: void *arg
// Uso: Esta función actualiza los datos del usuario desde el archivo de cuentas. 
//      Lee la información del archivo, buscando la cuenta correspondiente al usuario, 
//      y actualiza su saldo y número de transacciones en la RAM/local, para asi avitar probelmas de sincronoizacion
...
void actualizarDatos()
{
    ...
    FILE * fichero;
    char linea[255],error[255];
    char *id,*nombre,*saldo,*transacciones;
   
    //Bloqueamos el acceso al recurso
    sem_wait(semaforo_cuentas);
    semaforos_usados[1]=1;
    if ((fichero = fopen(User.ficheroCuentas,"r"))== 0)
    {
        //en caso de errro notificamos al usaurio y al log
        snprintf(error,sizeof(error),"Error %d del usuario con PID %d al abrir el fichero %s",errno,pid_programa,User.ficheroCuentas);
        fprintf(stderr,"%s\n",error);
        escribirLog(error);
        ...
        semaforos_usados[1]=0;
        //Si no puedo actulziar datos en un fallo critico asi que me cierrro
        kill(pid_programa,SIGTERM);
    ...
    while (fgets(linea,255,fichero) != NULL)
    {
        //Quitamos el /n
        linea[strcspn(linea,"\n")] = '\0';
        //separamos los datos por cadenas para hacer mas facil la busqueda
        nombre = strtok(linea,";");
        id = strtok(NULL,";");
        saldo = strtok(NULL,";");
        ...
        if (User.num_cuenta == aux)
        {
            //actualizo los datos de memoria
            User.saldo = atoi(saldo);
            User.transacciones = atoi(transacciones);
            break;
        }
...
bool buscarUser(int id)
{
   ...
   //creamos las variables
    ...
    bool encontrado;
    char linea[255],error[255];
    char *id_String, * nombre_string,*saldo_string;

    //Bloqueamos el acceso al fichero de cuentas para evitar problemas 
    sem_wait(semaforo_cuentas);
    semaforos_usados[1]=1;
    if ( (fichero = fopen(User.ficheroCuentas,"r"))== NULL)
    {
        snprintf(error,sizeof(error),"Error %d del usuario con PID %d al abrir el fichero %s",errno,pid_programa,User.ficheroCuentas);
        fprintf(stderr,"%s\n",error);
        escribirLog(error);
        ...
        sem_post(semaforo_cuentas);
        semaforos_usados[1]=0;
        //Si no puedo encontrar usuarios , no puedo hacer transferencias en un fallo critico asi que me cierrro
        kill(pid_programa,SIGTERM);
    }
    ...
    while (fgets(linea,255,fichero)!= NULL)
    {
        //Quito el \n
        linea[strcspn(linea,"\n")]= '\0';
        //Guardo los datos de cada linea para las comparaciones
        nombre_string = strtok(linea,";");
        id_String = strtok(NULL,";");
      

        //veo si hay coincidencias y devuelvo un true 
        if ( id == atoi(id_String))
        {
            sem_post(semaforo_cuentas);
            semaforos_usados[1]=0;
            return true;
