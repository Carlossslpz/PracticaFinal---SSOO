...
pid_t pidgeneral;
sem_t * semaforo_hilos,*semaforo_config,*semaforo_cuentas,*semaforo_trans,*semaforo_log,*semaforo_fifo;
...
int main(int argc, char* argv[])
{
    ...
    if (stat(PROPS.archivo_cuentas,&st) == -1)
    {
        ...
        if (fork()==0)
        {
            execl("./init_cuenta","init_cuenta",PROPS.archivo_cuentas,"1",PROPS.archivo_log,(char *)NULL);
        }
...
void  loginUsuario()
{
    ...
    char comando[1024],mensaje[255],n_cuenta[255],line[255];
    ...
    {
        //En caso de errno notificamos
        snprintf(mensaje,sizeof(mensaje),"Error %d al abrir el fichero %s\n",errno,PROPS.archivo_cuentas);
        escribirLog(mensaje);
        fprintf(stderr,"%s\n",mensaje);
    ...
    while (fgets(line,255,fichero) != NULL)
    {
        if (saltar)
        {
            saltar = false;
        ...
        if (strcmp (n_cuenta,cuenta) == 0)
        {
            ...
            snprintf(comando, sizeof(comando), "./usuario %s %s %s %s %s %s %s" 
                ,usuario,cuenta,saldo,transacciones,PROPS.archivo_transacciones,PROPS.archivo_cuentas,PROPS.archivo_log);
    ...
    if (encontrado)
    {
        ...
        {   
            ...
            if ( execvp("gnome-terminal",parametros)== -1 )
            {
               ...
               escribirLog("Error al abrir el programa de usuario");
        ...
        else if (pid < 0)
        {
            ...
            escribirLog("Fork de usuario ha fallado");
    ...
    else 
    {
        ...
        escribirLog("Inicio de sesion fallido");    
    }
