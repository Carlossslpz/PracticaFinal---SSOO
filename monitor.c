...
typedef struct 
{
    ...
    //Aqui guardo una tupla con el id de usuario y las veces que ha tranferido ej;
    //[1][2] al usuario 1 le ha tranferido 2 veces , tal que mas de 3 por sesion es fraude 
...
//Retorno: void.
//Parámetros: Ninguno.
//Uso: Lee el fichero de transacciones global, extrae las fechas y los IDs de los usuarios, 
...
void generarListasDatos()
{
    ...
    listaUsuarios.index = 0;
    listaFechas.index=  0;

    //Obtenemos los datos de las lineas
    while (fgets(linea,sizeof(linea),fichero) != NULL)
    {
        //Para cada linea veremos si la fecha y el id de usuario ya existen
        esta_en_fecha = false;
        esta_en_users = false;
       ...
       //Vemos si la fecha ya existe en la lista de fechas
        ...
        //Vemos si el id de usuario ya existe en la lista de usuarios
        for ( i = 0; i<listaUsuarios.index;i++)
        {
            if(listaUsuarios.lista[i].id == atoi(id))
            {
                esta_en_users = true;
                break;
            }                   
        }
        //Si no existe el id de usuario lo añadimos
      
        if (!esta_en_users) listaUsuarios.lista[listaUsuarios.index++].id = atoi(id); 
...
void * buscarIngresosMaximos(void * arg)
{
    ...
    for ( i = 0; i<listaFechas.index;i++)
    {
        ...
        //Reiniciamos los contadores de ingresos de usuariopara cada fecha
        for ( j = 0 ; j<listaUsuarios.index;j++)
        {
            listaUsuarios.lista[j].cantidad_ingresada = 0;
        }
        ...
        while (fgets(linea,sizeof(linea),fichero) != NULL)
        {
            ...
            {
                //Vemos si el id de usuario ya existe en la lista de usuarios
                if (listaUsuarios.lista[j].id == atoi(id))
                {
                    //Si existe lo sumamos a la cantidad ingresada
                    listaUsuarios.lista[j].cantidad_ingresada += atof(cantidad);

                    //Si el conjunto de cantidad ingresada es mayor al maximo lo notificamos
                    if (listaUsuarios.lista[j].cantidad_ingresada >= maximos.maxima_cantidad_ingreso)
                    {
    ...
    for ( i = 0; i<listaFechas.index;i++)
    {
        ...
        while (fgets(linea,sizeof(linea),fichero) != NULL)
        {
            ...
            for ( j = 0; j<listaUsuarios.index;j++)
            {
                ...
                {
                    //Si existe lo sumamos a la cantidad ingresada
                    listaUsuarios.lista[j].numero_ingresos +=1;

                    //Si el conjunto de cantidad ingresada es mayor al maximo lo notificamos
                    if (listaUsuarios.lista[j].numero_ingresos >= maximos.maximo_ingresos)
                    {
                        //Si el ingreso es mayor al maximo lo notificamos
                        snprintf(mensaje,sizeof(mensaje),"1-ALERTA: USUARIO CON ID %s HA SUPERADO EL MAXIMO NUMERO DE INGRESOS EL DIA %s\n",id,fecha);
                        escribirBanco(mensaje);    
                    }
                }
            }
...
void * buscarRetirosMaximos(void * arg)
{
    ...
    for ( i = 0; i<listaFechas.index;i++)
    {
        ...
        for ( j = 0 ; j<listaUsuarios.index;j++)
        {
            listaUsuarios.lista[j].cantidad_retirada = 0;
        }
        ...
        while (fgets(linea,sizeof(linea),fichero) != NULL)
        {
            ...
            for ( j = 0; j<listaUsuarios.index;j++)
            {
                ...
                {                 
                    ...
                    listaUsuarios.lista[j].cantidad_retirada += atof(cantidad);

                    //Si el conjunto de cantidad retirada es mayor al maximo lo notificamos
                    if (listaUsuarios.lista[j].cantidad_retirada >= maximos.maxima_cantidad_retiro)
                    {
                        //Si el ingreso es mayor al maximo lo notificamos
...
void * buscarTotalRetiros(void * arg)
{
    ...
    for ( i = 0; i<listaFechas.index;i++)
    {
        ...
        for ( j = 0 ; j<listaUsuarios.index;j++)
        {
            listaUsuarios.lista[j].num_retiros = 0;
        }
        ...
        while (fgets(linea,sizeof(linea),fichero) != NULL)
        {
            ...
            for ( j = 0; j<listaUsuarios.index;j++)
            {
                ...
                if (listaUsuarios.lista[j].id == atoi(id))
                {
                    ...
                    listaUsuarios.lista[j].num_retiros +=1;

                    //Si el conjunto de cantidad ingresada es mayor al maximo lo notificamos
                    if (listaUsuarios.lista[j].num_retiros >= maximos.maximo_retiros)
                    {
                        //Si el numero de retiros al dia el mayor al mexmio
                        snprintf(mensaje,sizeof(mensaje),"1-ALERTA: USUARIO CON ID %s HA SUPERADO EL MAXIMO NUMERO DE RETIROS EL DIA %s\n",id,fecha);
                        escribirBanco(mensaje);             
                    }
...
void * buscarTransferenciasMaximas(void * arg)
{
    ...
    for ( i = 0; i<listaFechas.index;i++)
    {
        ...
        for ( j = 0 ; j<listaUsuarios.index;j++)
        {
            listaUsuarios.lista[j].cantidad_transferida= 0;
        }
        ...
        while (fgets(linea,sizeof(linea),fichero) != NULL)
        {
            ...
            for ( j = 0; j<listaUsuarios.index;j++)
            {
                ...
                if (listaUsuarios.lista[j].id == atoi(id))
                {
                    ...
                    listaUsuarios.lista[j].cantidad_transferida += atof(cantidad);

                    //Si el conjunto de cantidad trabferida es mayor al maximo lo notificamos
                    if (listaUsuarios.lista[j].cantidad_transferida>= maximos.maxima_cantidad_transferencia)
                    {
                        //Si el la cantidad tranferida es mayor al maximo lo notificamos
                        snprintf(mensaje,sizeof(mensaje),"1-ALERTA: USUARIO CON ID %s HA SUPERADO LA MAXIMA CANTIDAD DE DINERO TRANSFERIBLE EL DIA %s\n",id,fecha);
                        escribirBanco(mensaje);                 
                    }
                }
            }
        ...
        while (fgets(linea,sizeof(linea),fichero) != NULL)
        {
            ...
            for ( j = 0; j<listaUsuarios.index;j++)
            {
                ...
                if (listaUsuarios.lista[j].id == atoi(id))
                {
                    ...
                    listaUsuarios.lista[j].num_transferencias +=1;

                    //Si la suma de transferencias es mayor al maximo lo notificamos
                    if (listaUsuarios.lista[j].num_transferencias>= maximos.maximo_tranferencias)
                    {
                    
                        snprintf(mensaje,sizeof(mensaje),"1-ALERTA: USUARIO CON ID %s HA SUPERADO EL MAXIMO NUMERO DE TRANSFERENCIAS EL DIA %s\n",id,fecha);
                        escribirBanco(mensaje);              
                    }
                }
            }
...
void * buscarTransferenciasEntreCuentas(void * arg)
{
    ...
    for ( i = 0; i<listaFechas.index;i++)
    {
        ...
        while (fgets(linea,sizeof(linea),fichero) != NULL)
        {
            ...
            for ( j = 0; j<listaUsuarios.index;j++)
            {
                //VIdentificamos si el id de usuario ya existe en la lista de usuarios
                if (listaUsuarios.lista[j].id == atoi(id))
                {
                    //Para cada usuario miramos su lista de tranferidoas a ver si h atransferido a alguien
                    for ( k = 0; k<=listaUsuarios.lista[j].n_usus_trans;k++)
                    {
                        if (listaUsuarios.lista[j].usuarios_transferidos[k][0] == atoi(id_tranf))
                        { 
                            encontrado = true;
                            break;
                        }
                    ...
                    if (encontrado)
                    {
                        ...
                        listaUsuarios.lista[j].usuarios_transferidos[k][1] += 1;

                        //Si el numero de transferencias entre cuentas es mayor al maximo lo notificamos
                        if (listaUsuarios.lista[j].usuarios_transferidos[k][1] >= maximos.maximo_transferencias_entre_cuentas)
                        {
                    ...
                    else 
                    {
                        ...
                        listaUsuarios.lista[j].usuarios_transferidos[listaUsuarios.lista[j].n_usus_trans++][0] = atoi(id_tranf);
                        listaUsuarios.lista[j].usuarios_transferidos[listaUsuarios.lista[j].n_usus_trans][1] += 1;
                    }
                    break;
                }
            }
